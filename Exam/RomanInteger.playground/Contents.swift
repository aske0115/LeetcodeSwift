...
class Solution {
    func romanToInt(_ s: String) -> Int {
        let dictionary:[String: String] = ["I" : "1",
                                           "IV" : "4",
                                           "V" : "5",
                                           "IX" : "9",
                                           "X" : "10",
        ...
        func isRoman(_ char:String) -> Bool {
            char == "I" || char == "X" || char == "C"
        }
        ...
        for i in 0..<strArray.count {
            ...
            print("for start \(strArray[i])")
            if isRoman(strArray[i]) {
                if i < strArray.count - 1, let value = dictionary[strArray[i...i+1].joined(separator:"")] {
                    print("if for \(value)")
                    result += Int(value)!
                    pass = true
                } else {
                    print("else for \(dictionary[strArray[i]])")
                    result += Int(dictionary[strArray[i]]!)!
                }
            } else {
                print("else  \(dictionary[strArray[i]])")
                result += Int(dictionary[strArray[i]]!)!
            }
            print("result = \(result)")
        }
